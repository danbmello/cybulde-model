...
def save_config_as_yaml(config: Union["Config", DictConfig], save_path: str) -> None:
    # Creates a StringIO and writes some text.
    text_io = StringIO()
    text_io.writelines(
        [
    ...
    # Load cybulde/configs/automatically_generated/full_config_header.yaml
    # full_config_header.yaml has the same configs of cybulde/configs/config.yaml
    # We need to 'manually' add this configuration parameters to our final config.
    config_header = load_config_header()
    text_io.write(config_header)
    text_io.write("\n")

    # Save the config file to sav_path
