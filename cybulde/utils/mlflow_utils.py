...
# MLFlow tracking server read from the environment variable MLFLOW_TRACKING_URI.
...
# Context manager designed to manage the lifecycle of an MLflow run.
...
# Set experiment function
def set_experiment(experiment_name: Optional[str] = None) -> None:
    if experiment_name is None:
        experiment_name = "Default"

    # If the experiment exists we won't be able to create again, that's why we use a try/except statement.
    try:
